...
export async function dueUpdateRequest(transactionid, dueUpdateState, token) {
  if(token) {
    try {
      const {data} = await axios.patch(`http://localhost:7272/api/transaction/due/edit/${transactionid}`,
        dueUpdateState,
        {
          headers: {
            'authorization': token
